...
let showImagesBtn = document.getElementById('show-images-btn');
...
let filterButtons = document.getElementById('filter-btn');
...
// Графіки
// Кругова діаграма
let maleProducts = images.filter((item) => item.classname === 'male').length;
let femaleProducts = images.filter((item) => item.classname === 'female').length;
...
// Гістограма
let prices = images.map((item) => parseFloat(item.price));
new Chart("barChart", {
	type: 'bar',
	data: {
...
// Лінійний графік
let maleProductsCount = [];
let femaleProductsCount = [];
images.forEach((item, index) => {
	let maleCount = images.slice(0, index + 1).filter((item) => item.classname === 'male').length;
	let femaleCount = images.slice(0, index + 1).filter((item) => item.classname === 'female').length;
